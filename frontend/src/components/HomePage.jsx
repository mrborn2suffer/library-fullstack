import React from 'react';

const HomePage = ({ books, onAddToCart }) => {
  return (
    <section className="mono-grid">
      {books.map(book => (
        <article key={book.id} className="mono-card mono-book-card">
          <div className="mono-book-cover">
            {book.coverImageUrl ? (
              <img src={book.coverImageUrl} alt={book.title} />
            ) : (
              <div className="mono-cover-placeholder">{book.title[0]}</div>
            )}
          </div>
          <div className="mono-book-body">
            <h2>{book.title}</h2>
            <p className="mono-book-meta">
              {book.author} · {book.genre}
            </p>
            <p className="mono-book-desc">{book.description}</p>
            <p className="mono-book-price">₹{book.marketPrice}</p>
            <button
              className="mono-button primary small"
              onClick={() => onAddToCart(book.id)}
            >
              Add to cart
            </button>
          </div>
        </article>
      ))}
      {books.length === 0 && (
        <div className="mono-empty">No books found.</div>
      )}
    </section>
  );
};

export default HomePage;

